...
class ApiResourceGenerator extends StatementGenerator implements Generator
{
    const INDENT = '            ';
    ...
    protected function populateStub(string $stub, Controller $controller, ResourceStatement $resource): string
    {
        ...
        $isCollection = $resource->collection() && $resource->generateCollectionClass();
        $imports[] = $isCollection ? 'use Illuminate\\Http\\Resources\\Json\\ResourceCollection;' : 'use Illuminate\\Http\\Resources\\Json\\JsonResource;';

        $stub = str_replace('{{ namespace }}', $namespace, $stub);
        $stub = str_replace('{{ imports }}', implode(PHP_EOL, $imports), $stub);
        $stub = str_replace('{{ parentClass }}', $isCollection ? 'ResourceCollection' : 'JsonResource', $stub);
        $stub = str_replace('{{ class }}', $resource->name(), $stub);
        $stub = str_replace('{{ resource }}', $isCollection ? 'resource collection' : 'resource', $stub);

        // Add $collects property and PHPDoc for collections
        if ($isCollection) {
    ...
    protected function buildCollectsProperty(string $resourceClassName): string
    {
        return '    /**'.PHP_EOL
            .'     * The resource that this resource collects.'.PHP_EOL
            .'     *'.PHP_EOL
            .'     * @var string'.PHP_EOL
            .'     */'.PHP_EOL
